...
def visit(path_so_far, current, double_cave, map):
    ...
    if current == 'end':
        ...
        for next in map[current]:
            if next in map:
                if current.isupper():
                    paths += visit(updated_path_so_far, next, double_cave, map)
                elif current not in path_so_far:
                    paths += visit(updated_path_so_far, next, double_cave, map)
                elif not double_cave:
                    paths += visit(updated_path_so_far, next, current, map)
        return paths
